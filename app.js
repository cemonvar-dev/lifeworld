	...
	Object.keys(groups).sort((a, b) => groupLabel(a).localeCompare(groupLabel(b))).forEach(tag => {
		...
		groups[tag].forEach(tile => {
			...
			const healthHeader = tp.finished
				? `<div class="-mt-4 -mx-4 mb-3 h-16 rounded-t-xl bg-slate-100 flex items-center justify-center text-3xl" style="width:calc(100% + 2rem)" title="${tp.label}">${tp.emoji}</div>`
				: `<div class="-mt-4 -mx-4 mb-3 h-16 rounded-t-xl bg-cover bg-bottom" style="width:calc(100% + 2rem);background-image:url('styling/icons/weather/${tierLabel}.jpg')" title="${tp.label}"></div>`;
			tileDiv.innerHTML = `
				   ${healthHeader}
